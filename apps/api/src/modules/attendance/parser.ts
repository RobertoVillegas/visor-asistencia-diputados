...
const orderedStatuses = [...statusMap.keys()].sort((a, b) => b.length - a.length);
...
function parseRecordLine(line: string) {
  for (const status of orderedStatuses) {
    const suffixPattern = new RegExp(`\\s+${status.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i");
...
export function parseAttendancePages(pages: string[]): ParsedAttendanceDocument {
  ...
  for (const record of records) {
    ...
    if (record.status === "justified_absence") existing.justifiedAbsenceCount += 1;
    if (record.status === "absence") existing.absenceCount += 1;
    if (record.status === "official_commission") existing.officialCommissionCount += 1;
    if (record.status === "board_leave") existing.boardLeaveCount += 1;
    if (record.status === "not_present_in_votes") existing.notPresentInVotesCount += 1;
