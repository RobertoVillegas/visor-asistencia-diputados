...
import { normalizeName, parseAttendancePages } from "./parser";
import {
  ...
  try {
    ...
    await db
      ...
      .set({
        ...
        lastChangedAt: changed || !previousSnapshot ? fetchedAt : previousSnapshot?.changedAt ?? null,
      })
...
export async function reconcileSessionAbsences(sessionId: string) {
  ...
  const absenceByName = new Map(parsedAbsences.records.map((record) => [record.normalizedName, record]));
  ...
  const absenceCounts = new Map(parsedAbsences.summaries.map((summary) => [summary.groupCode, summary.absenceCount]));
...
export async function parseAndPersistAttendanceDocument(sessionId: string) {
  ...
  const legislature = row.period?.legislature ?? inferLegislatureFromSessionUrl(row.session.sessionPageUrl);
  ...
  return db.transaction(async (tx) => {
    ...
    await tx.delete(sessionGroupSummaries).where(eq(sessionGroupSummaries.sessionId, row.session.id));
...
export async function listLegislators(
  ...
  const enriched = rows.map((row) => {
    ...
    const absenceRatio = row.sessionsMentioned > 0 ? row.absenceCount / row.sessionsMentioned : 0;
  ...
  const sortMap: Record<LegislatorSort, (item: (typeof enriched)[number]) => string | number | null> = {
...
export async function getAnalyticsOverview(scope: AnalyticsScope = {}) {
  ...
  if (scope.legislature) sessionWhere.push(eq(legislativePeriods.legislature, scope.legislature));
  const combinedWhere =
    sessionWhere.length === 0 ? undefined : sessionWhere.length === 1 ? sessionWhere[0] : and(...sessionWhere);
  ...
  return {
    ...
    ...statusSummary,
    attendanceRatio:
      statusSummary.totalMentions > 0 ? statusSummary.attendanceCount / statusSummary.totalMentions : 0,
    absenceRatio:
      statusSummary.totalMentions > 0 ? statusSummary.absenceCount / statusSummary.totalMentions : 0,
...
export async function listPartyAnalytics(scope: AnalyticsScope = {}, order: SortOrder = "desc") {
  ...
  if (scope.legislature) clauses.push(eq(parliamentaryGroups.legislature, scope.legislature));
  const whereClause =
  ...
  const enriched = rows.map((row) => ({
    ...
    absenceRatio: row.totalCount > 0 ? row.absenceCount / row.totalCount : 0,
...
export async function getLegislatorTrend(legislatorId: string, scope: AnalyticsScope = {}) {
  ...
  if (scope.periodId) clauses.push(eq(sessions.periodId, scope.periodId));
  if (scope.legislature) clauses.push(eq(legislators.legislature, scope.legislature));
...
export async function getSessionComposition(sessionId: string) {
  ...
  return {
    ...
    parties: parties.map((party) => ({
      ...
      absenceRatio: party.totalCount > 0 ? party.absenceCount / party.totalCount : 0,
      justifiedAbsenceRatio:
...
export async function getQualityOverview(scope: AnalyticsScope = {}) {
  const clauses = [];
  if (scope.periodId) clauses.push(eq(sessions.periodId, scope.periodId));
  if (scope.legislature) clauses.push(eq(legislativePeriods.legislature, scope.legislature));
  const whereClause =
    clauses.length === 0 ? undefined : clauses.length === 1 ? clauses[0] : and(...clauses);
  ...
  return {
    scope,
    ...summary,
    parseCoverageRatio: summary.totalSessions > 0 ? summary.parsedSessions / summary.totalSessions : 0,
    reconciliationCoverageRatio:
      summary.totalSessions > 0 ? summary.reconciledSessions / summary.totalSessions : 0,
    matchRatio: summary.reconciledSessions > 0 ? summary.matchedSessions / summary.reconciledSessions : 0,
  };
}
...
export async function listSessionQuality(scope: AnalyticsScope = {}) {
  ...
  const rows = await db
    .select({
      ...
      periodId: sessions.periodId,
      periodLabel: legislativePeriods.label,
      attendanceDocumentId: sql<string | null>`max(case when ${sessionDocuments.kind} = 'attendance' then ${sessionDocuments.id}::text else null end)`,
      absenceDocumentId: sql<string | null>`max(case when ${sessionDocuments.kind} = 'absence' then ${sessionDocuments.id}::text else null end)`,
  ...
  return rows.map((row) => ({
    ...row,
    parseStatus:
      row.attendanceRecordCount > 0 ? "parsed" : row.attendanceDocumentId ? "discovered" : "missing_attendance_document",
