...
app.get("/api/sessions/parsed", async (c) =>
  c.json(await getSessionsWithParsedAttendance())
)
...
app.get("/api/legislators", async (c) => {
  ...
  return c.json(
    await listLegislators(search, { legislature, periodId }, sort, order)
  )
})
...
app.get("/api/analytics/overview", async (c) => {
  ...
  return c.json(
    await getAnalyticsOverview({ legislature, periodId, includePermanent })
  )
})
...
app.get("/api/analytics/parties", async (c) => {
  ...
  return c.json(
    await listPartyAnalytics({ legislature, periodId, includePermanent }, order)
  )
})
...
app.get("/api/analytics/trends/parties", async (c) => {
  ...
  const periodId = c.req.query("periodId")
  const includePermanent = c.req.query("includePermanent") === "true"
  return c.json(await getPartyTrends({ legislature, periodId, includePermanent }))
})
...
app.post("/api/crawl/extract-session-document", requireAdmin, async (c) => {
  ...
  try {
    const result = await extractAndPersistSessionDocument(
      body.sessionId,
      body.kind
    )
    return c.json(result, 201)
...
app.post("/api/crawl/snapshot-session-document", requireAdmin, async (c) => {
  ...
  try {
    const result = await createSessionDocumentSnapshot(
      body.sessionId,
      body.kind
    )
    return c.json(result, 201)
...
app.post("/api/crawl/parse-period-attendance", requireAdmin, async (c) => {
  ...
  try {
    ...
    return c.json(
      {
        error:
          error instanceof Error ? error.message : "Unknown batch parse error",
      },
      400
    )
  }
    ...
    return c.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unknown batch reconciliation error",
      },
      400
    )
  }
...
app.post("/api/auth/sign-up/email", async (c) => {
  ...
  if (!adminEmails.has(email)) {
    return c.json(
      { error: "Only configured admin emails may create accounts." },
      403
    )
  }
