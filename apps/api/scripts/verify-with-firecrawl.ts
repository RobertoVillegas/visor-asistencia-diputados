...
import { attendanceRecords, legislators, sessionDocuments, sessions } from "../src/db/schema";
import { env } from "../src/env";
import { normalizeName, parseAttendancePages } from "../src/modules/attendance/parser";
...
async function main() {
  ...
  const [attendanceDoc] = await db
    ...
    .where(and(eq(sessionDocuments.sessionId, sessionId), eq(sessionDocuments.kind, "attendance")))
    .limit(1);
  ...
  const dbMap = new Map(
    dbRows.map((row) => [row.normalizedName, { rawName: row.rawName, status: row.status }]),
  );
  ...
  if (!response.ok || payload.success === false) {
    console.error("Firecrawl request failed:", payload.error ?? response.statusText);
    process.exit(1);
  }
  ...
  const fcMap = new Map(
    parsed.records.map((record) => [
      ...
      { rawName: record.rawName, status: record.status },
    ]),
  );
  ...
  console.log(`Only in Firecrawl (${onlyInFirecrawl.length}):`, onlyInFirecrawl.slice(0, 25));
  console.log(`Status mismatches (${statusMismatches.length}):`, statusMismatches.slice(0, 25));

  if (onlyInDb.length === 0 && onlyInFirecrawl.length === 0 && statusMismatches.length === 0) {
