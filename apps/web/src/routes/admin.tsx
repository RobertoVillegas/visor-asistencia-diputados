...
function AdminPage() {
  ...
  const [peopleDirectory, setPeopleDirectory] = useState<PeopleDirectoryResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadPeriods() {
      try {
        ...
        setSelectedPeriodPageUrl((current) => current || nextPeriods[0]?.periodPageUrl || "");
        setSelectedLegislature((current) => current || nextPeriods[0]?.legislature || "LXVI");
      } catch (caughtError) {
        if (!cancelled) {
          setStatus(
  ...
  async function handleEmailAuth() {
    ...
    try {
      ...
      setStatus(caughtError instanceof Error ? caughtError.message : "Falló la autenticación.");
    } finally {
  ...
  async function handleSignOut() {
    ...
    try {
      ...
      if (response.error) {
        throw new Error(response.error.message || "No se pudo cerrar la sesión.");
      }
      ...
      setStatus(
        caughtError instanceof Error ? caughtError.message : "No se pudo cerrar la sesión.",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  async function runProcess(body: {
    ...
    try {
      ...
      setStatus(caughtError instanceof Error ? caughtError.message : "El procesamiento falló.");
    } finally {
      setIsSubmitting(false);
    }
  }
  ...
  async function runProcessAll(body: { legislature?: string }) {
    ...
    try {
      ...
      setStatus(
        caughtError instanceof Error ? caughtError.message : "El procesamiento masivo falló.",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const selectedPeriod = periods.find((period) => period.periodPageUrl === selectedPeriodPageUrl);
  const visiblePeriods = periods.filter((period) => period.legislature === selectedLegislature);
  const selectedPerson =
    peopleDirectory?.items.find((item) => item.legislatorId === selectedPersonId) ?? null;
  ...
  useEffect(() => {
    ...
    async function loadAdminData() {
      try {
        ...
        if (!cancelled) {
          setStatus(
            caughtError instanceof Error ? caughtError.message : "No se pudo cargar la auditoría.",
          );
        }
      }
    }
  ...
  useEffect(() => {
    ...
    async function loadPeople() {
      try {
        ...
        setSelectedPersonId((current) => current ?? nextPeople.items[0]?.legislatorId ?? null);
      } catch (caughtError) {
        if (!cancelled) {
          setStatus(
            caughtError instanceof Error
  ...
  async function handleCleanupInvalidGroups() {
    ...
    try {
      const cleanup = await api.cleanupInvalidGroups({
        legislature: selectedLegislature,
      });
      setCleanupResult(cleanup);
      setStatus(`Limpieza completada: ${cleanup.deleted} grupos inválidos eliminados.`);
    } catch (caughtError) {
      setStatus(
        caughtError instanceof Error ? caughtError.message : "No se pudo ejecutar la limpieza.",
      );
    } finally {
      setIsSubmitting(false);
    }
        ...
        <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
              Esta ruta no se muestra en la navegación pública. Sirve para autenticar
              administradores y disparar el pipeline de descubrimiento, parseo y conciliación.
            </p>
          </div>
          ...
          {!isAuthenticated ? (
            <div className="mt-8 border-t border-border pt-8">
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Solo los correos admin configurados en el backend pueden crear cuenta. Contacta al
          ...
          <>
            <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Usa la lista oficial completa. Si el periodo no está importado, se descubre y
                procesa; si ya está importado, se refresca.
              </p>
              <label className="mt-5 flex flex-col gap-2 text-sm">
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    const nextPeriod = periods.find((period) => period.legislature === legislature);
              ...
              <label className="mt-5 flex flex-col gap-2 text-sm">
                ...
                <select
                  className="h-11 rounded-2xl border border-border bg-card px-4"
                  onChange={(event) => setSelectedPeriodPageUrl(event.target.value)}
            ...
            <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
              <h2 className="font-heading text-2xl text-foreground">Descubrir desde URL</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Úsalo para cargar una nueva página anual de asistencia de la legislatura.
              </p>
              <label className="mt-5 flex flex-col gap-2 text-sm">
                URL del periodo
                <input
            ...
            <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
              <h2 className="font-heading text-2xl text-foreground">Procesamiento masivo</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Recorre todos los periodos publicados o todos los de una legislatura. Esto es la
                base para correr un cronjob después.
              </p>
              <div className="mt-5 flex flex-wrap items-end gap-3">
                <label className="flex flex-col gap-2 text-sm">
                  ...
                  <select
                    className="h-11 rounded-2xl border border-border bg-card px-4"
                    onChange={(event) => setSelectedLegislature(event.target.value)}
                    value={selectedLegislature}
                  >
                    {availableLegislatures.map((legislature) => (
                      <option key={legislature} value={legislature}>
                ...
                <Button
                  disabled={isSubmitting}
                  onClick={() => void runProcessAll({ legislature: selectedLegislature })}
                >
              ...
              <div className="mt-4 flex items-center gap-3">
                <label className="flex cursor-pointer items-center gap-2 text-sm">
                  ...
                  <span
                    className={
                      forceParseAll ? "font-medium text-amber-700" : "text-muted-foreground"
                    }
                  >
            ...
            <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
              <h2 className="font-heading text-2xl text-foreground">Cola y jobs</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Estado en tiempo real del worker, los jobs encolados y el resultado de cada corrida.
              </p>

              <div className="mt-5 grid gap-3 sm:grid-cols-4">
                <MiniStat label="Pendientes" value={String(jobs?.stats.pending ?? 0)} />
                <MiniStat label="Corriendo" value={String(jobs?.stats.running ?? 0)} />
                <MiniStat label="Completados" value={String(jobs?.stats.completed ?? 0)} />
                <MiniStat label="Fallidos" value={String(jobs?.stats.failed ?? 0)} />
              </div>
              ...
              <div className="mt-5 max-h-[28rem] overflow-auto rounded-3xl border border-border/70">
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody>
                    {jobs?.jobs.map((job) => (
                      <tr className="border-t border-border/70 align-top" key={job.id}>
            ...
            <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
              <h2 className="font-heading text-2xl text-foreground">Sesiones sospechosas</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Detecta sesiones con pocos registros, conciliación pendiente o mismatch.
              </p>
                ...
                {suspiciousSessions.slice(0, 24).map((session) => (
                  ...
                  >
                    ...
                    <p className="mt-2 text-sm text-foreground">
                      {translateQualityStatus(session.reconciliationStatus)} · registros{" "}
                      {session.attendanceRecordCount}
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      Parseo: {translateParseStatus(session.parseStatus)} · difs:{" "}
                      {session.groupDiffCount}
                    </p>
                  </article>
                ))}
              </div>
            ...
            <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Advertencias del parseo: estatus o grupos desconocidos, parser compacto, etc.
                  </p>
                </div>
              ...
              <div className="mt-5 max-h-[28rem] space-y-3 overflow-auto">
                {ingestAnomalies.length === 0 ? (
                  <p className="text-sm text-muted-foreground">Sin anomalías recientes.</p>
                ) : (
                  ingestAnomalies.map((anomaly) => (
                    ...
                    >
                      ...
                      <p className="mt-2 text-sm text-foreground">{anomaly.message}</p>
                      {anomaly.snippet ? (
                        <p className="mt-2 text-xs text-muted-foreground">{anomaly.snippet}</p>
                      ) : null}
                      {anomaly.sourceUrl ? (
                        <a
            ...
            <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
              <h2 className="font-heading text-2xl text-foreground">Limpieza de datos</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Elimina grupos parlamentarios inválidos creados por errores de parseo (nombres de
                personas, códigos largos, etc.).
              </p>
              <div className="mt-5">
                <Button
                  disabled={isSubmitting}
              ...
              {cleanupResult ? (
                <div className="mt-4 rounded-2xl border border-border bg-muted/40 p-4">
                  <p className="text-sm font-medium">{cleanupResult.deleted} grupos eliminados:</p>
            ...
            <section className="rounded-[2rem] border border-border bg-card p-6 sm:p-8">
              <h2 className="font-heading text-2xl text-foreground">Perfiles públicos</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Enriquecimiento manual de imagen y bio para el directorio público.
              </p>
                ...
                <div className="space-y-4">
                  {selectedPerson ? (
                    <>
                      ...
                      <Button disabled={isSubmitting} onClick={() => void handleSaveProfile()}>
                        Guardar perfil
                      </Button>
                    </>
                  ) : (
...
function translateQualityStatus(status: SessionQualityRow["reconciliationStatus"]) {
  if (status === "matched") {
    return "conciliada";
  }
